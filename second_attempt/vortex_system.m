...
ts = 0.15; % maybe come up with a function for determining this
...
for n=1:NBlades % loop over blades
    ...
    cp.twist((n-1)*N+1:n*N) = interp1(r_R, twist_distribution, r/Radius); % twist at control points [deg]
    cp.x((n-1)*N+1:n*N) = zeros(N,1)+(0.5*c1'); % x-coordinate
    ...
    bound.z((n-1)*(N+1)+1:n*(N+1)) = r_R*Radius*sin(2*pi-blade_azim);     % z-coordinate2*pi-
    ...
    for i=1:N+1 % loop over bound vortices
        ...
        trailx((n-1)*(N+1)+i) = bound.x((n-1)*(N+1)+i)+0.75*c*cos(beta);
        traily((n-1)*(N+1)+i) = bound.y((n-1)*(N+1)+i)-0.75*c*sin(beta)*sin(blade_azim);
        trailz((n-1)*(N+1)+i) = bound.z((n-1)*(N+1)+i)+0.75*c*sin(beta)*cos(blade_azim);
